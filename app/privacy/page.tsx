...
export const metadata: Metadata = {
  title: 'プライバシーポリシー',
  description: '当サイトのプライバシーポリシーについて説明するページです。',
};

const PrivacyPolicyPage = () => {
  // ✅ この部分をあなたの情報に書き換えてください
  const operatorName = 'ホーム千郷';
  const departmentName = '運営事務局';
  const contactEmail = 'mail@homechisato.com';
  const effectiveDate = '2025年8月26日';

  return (
    <main style={{ padding: '2rem 1rem', maxWidth: '800px', margin: 'auto', lineHeight: '1.8' }}>
                  <div className="title-text">PrivacyPolicy</div>
      <p>
        {operatorName}（以下「当方」といいます。）は、当方が提供するウェブサイト（以下「当サイト」といいます。）における、ユーザーの個人情報の取扱いについて、以下のとおりプライバシーポリシー（以下「本ポリシー」といいます。）を定めます。
      </p>
      ...
      <h2>第2条（個人情報の収集方法）</h2>
      <p>
        当サイトでは、ユーザーが利用登録をする際に氏名、生年月日、住所、電話番号、メールアドレス、銀行口座番号、クレジットカード番号、運転免許証番号などの個人情報をお尋ねすることがあります。また、ユーザーと提携先などとの間でなされたユーザーの個人情報を含む取引記録や決済に関する情報を、当方の提携先（情報提供元、広告主、広告配信先などを含みます。以下、｢提携先｣といいます。）などから収集することがあります。
      </p>
      <p>
        当サイトでは、お問い合わせフォームやコメントの送信時に、氏名（ハンドルネーム）、メールアドレス等の個人情報を取得します。
      </p>
      ...
      <h2>第11条（広告配信について）</h2>
      <p>
        当サイトは第三者配信の広告サービス（Googleアドセンス、A8.netなど）を利用しています。このような広告配信事業者は、ユーザーの興味に応じた商品やサービスの広告を表示するため、当サイトや他サイトへのアクセスに関する情報 『Cookie』(氏名、住所、メール アドレス、電話番号は含まれません) を使用することがあります。 またGoogleアドセンスに関して、このプロセスの詳細やこのような情報が広告配信事業者に使用されないようにする方法については、
        <a href="https://policies.google.com/technologies/ads?gl=jp" target="_blank" rel="noopener noreferrer">
          こちら
        </a>
        をクリックしてください。
      </p>
      ...
      <ul style={{ listStyleType: 'none', paddingLeft: '0' }}>
        <li>事業者名：{operatorName}</li>
        <li>担当部署：{departmentName}</li>
        <li>Eメールアドレス：{contactEmail}</li>
      </ul>
